...
namespace joint_position_controller_server
{
   ...
   * the node's default MutuallyExclusive callback group, so they cannot run simultaneously.
   * The executable supplied by this package dispatches them through a `SingleThreadedExecutor`.
   *
  ...
  class JointPositionControllerServer: public rclcpp::Node
  {
    ...
    private:
       ...
       *
       * This callback runs at command_publication_frequency.
       * It reads pos_cmd_ under mutex protection.
      ...
      /** @brief Execution-loop rate for convergence, cancellation, and timeout checks, in Hertz. */
      ...
      /**
       * @brief True after receiving at least one valid JointState for the configured joint.
       */
      bool current_pos_is_valid_{false};

      /**
       * @brief Time when the latest valid JointState for the configured joint was received.
       */
       ...
       * slip through before ROS 2 later calls `handle_accepted()`. `execute()` clears it when that
