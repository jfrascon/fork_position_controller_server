...
def launch_fork_serial_node(ctx: LaunchContext) -> list[LaunchDescriptionEntity]:
    ...
    update_frequency = LaunchConfiguration('update_frequency').perform(ctx)
    ...
    if update_frequency:
        try:
            parameters.append({'update_frequency': float(update_frequency)})
        except ValueError as exc:
            raise ValueError(f"Invalid value for update_frequency: '{update_frequency}'. Expected a float.") from exc
